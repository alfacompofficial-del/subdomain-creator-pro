...
export default function IdeAiPanel({ currentCode, currentLang, fileName, onApply }: IdeAiPanelProps) {
  const [messages, setMessages] = useState<Message[]>([
    { role: "ai", text: "Привет! Я ИИ-помощник на базе Gemini 2.5 Flash. Опишите что нужно сделать с кодом, и я помогу!" }
  ]);
  ...
  const send = async () => {
    ...
    try {
      ...
      if (result) {
        setMessages(prev => [...prev, { role: "ai", text: `Готово! Применить изменения к файлу?\n\n\`\`\`\n${result.slice(0, 300)}${result.length > 300 ? "\n..." : ""}\n\`\`\`` }]);
        onApply(result);
        toast.success("ИИ обновил код");
      } else {
        setMessages(prev => [...prev, { role: "ai", text: "Не смог обработать запрос. Попробуйте иначе." }]);
      }
    } catch {
      setMessages(prev => [...prev, { role: "ai", text: "Ошибка при обращении к Gemini." }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex flex-col h-full bg-[#12141a] border-l border-white/8">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/8 bg-[#1a1d27] shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-white/80">Gemini AI</span>
          <span className="text-[10px] text-violet-400 bg-violet-500/10 px-1.5 py-0.5 rounded-full">2.5 Flash</span>
        </div>
        <Button size="icon" variant="ghost" className="h-6 w-6 text-white/30 hover:text-white/60"
          onClick={() => setMessages([{ role: "ai", text: "Чат очищен. Чем помочь?" }])}>
      ...
      <div className="px-3 py-2 border-t border-white/8 shrink-0">
        ...
        <p className="text-[10px] text-white/20 mt-1">Enter — отправить · Shift+Enter — перенос</p>
      </div>
    </div>
  );
}
